...
$(function () {
    ...
    var base = ['web','springboot', 'spring-test', 'logging', 'spock'];
    var common = [].concat(base);
    var data = ['h2', 'jdbc'];
    var data_jpa = ['data-jpa'];
    var pdx = ['predix-app', 'stuff'].concat(base);
    ...
    $("#archetype").on("change", function () {
        ...
        if (val === 'LIBRARY') {
            ...
            results = starters.get([].concat(_toConsumableArray(all)));
        } else if (val === 'pdx-microservice') {
            setDefaultPackageName('com.org.ge');
            //setArtifactIdAndBaseDir('rabbit-', '-service');
            results = starters.get([].concat(pdx));
        }  else if (val === 'pdx-alert-eventhub-subscriber') {
            setDefaultPackageName('com.org.ge');
            setArtifactIdAndBaseDir('', '-alert-eventhub-subscriber');
            results = starters.get(['predix-eventhub-subscriber'].concat(pdx));
        }  else if (val === 'pdx-timeseries-eventhub-subscriber') {
            setDefaultPackageName('com.org.ge');
            setArtifactIdAndBaseDir('', '-timeseries-eventhub-subscriber');
            results = starters.get(['predix-eventhub-subscriber'].concat(pdx));
        } else {
            setDefaultPackageName('com.org');
